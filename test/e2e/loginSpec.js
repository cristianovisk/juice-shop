...
describe('/#/login', () => {
  ...
  describe('challenge "loginCiso"', () => {
    it('should be able to log in as ciso@juice-sh.op by using "Remember me" in combination with (fake) OAuth login with another user', () => {
      ...
      browser.executeScript('var $http = angular.element(document.body).injector().get(\'$http\'); $http.post(\'/rest/user/login\', {email: \'admin@juice-sh.op\', password: \'***REMOVED***\', oauth: true});')
